...
namespace cluster_approx {
    ...
    PCSTFast::PCSTFast(const std::vector<std::pair<IndexType, IndexType>>& edges,
        ...
        : edges_(edges),
          ...
          logger_(logger ? std::move(logger) : NoOpLogger),
          verbosity_level_(verbosity_level)
    {
        ...
        if (root_ >= 0 && target_num_active_clusters > 0) {
            format_and_log(2, "Warning: target_num_active_clusters > 0 is ignored in the rooted case (using 0 internally).\n");
        }
    ...
    void PCSTFast::initialize_edges() {
        ...
        for (size_t ii = 0; ii < num_edges; ++ii) {
            ...
            if (uu < 0 || vv < 0 || static_cast<size_t>(uu) >= num_nodes || static_cast<size_t>(vv) >= num_nodes) {
                format_and_log(0, "Error: Edge %d endpoint index (%d or %d) out of range [0, %zu).\n", edge_index, uu, vv, num_nodes);
            ...
            if (uu == vv) {
                format_and_log(2, "Warning: Ignoring self-loop edge %d (%d -> %d).\n", edge_index, uu, vv);
    ...
    bool PCSTFast::run(std::vector<IndexType>* result_nodes, std::vector<IndexType>* result_edges) {
        ...
        while (current_num_active_clusters > target_num_active_clusters_) {
            ...
            if (event.type == NextEvent::Type::NONE) {
                format_and_log(2, "No more events possible. Terminating GW loop prematurely.\n");
                break;
            }

            format_and_log(4, "-----------------------------------------\n");
            if (event.time < current_time_ - kEpsilon) {
                format_and_log(0, "CRITICAL Error: Time decreased! Current=%.9g, Event=%.9g. Algorithm state likely corrupt.\n", current_time_, event.time);
                assert(false && "Time decreased in main loop");
                return false;
            }
            current_time_ = event.time;
            format_and_log(4, "Advanced time to %.9g\n", current_time_);
            ...
            format_and_log(3, "End of event processing iteration %d. Active representative clusters: %d (was %d)\n", event_count, current_num_active_clusters, previous_active_count);
        }

        format_and_log(3, "Finished GW main loop: Final time %.9g, Total edge events processed %lld, Final active representative clusters %d\n",
                       current_time_, stats_.total_num_edge_events, current_num_active_clusters);
        format_and_log(3, "--- Starting Pruning and Output Generation ---\n");
        format_and_log(3, "Selecting initial 'good' nodes based on final active/root clusters...\n");
        ...
        format_and_log(3, "Found %d 'good' nodes connected to final structure.\n", good_node_count);
        ...
        format_and_log(3, "Calling selected pruner strategy's prune method...\n");
        assert(pruner_ && "Pruner strategy object is null!");
        try {
            pruner_->prune(context, *result_nodes, *result_edges);
        } catch (const std::exception& e) {
            format_and_log(0, "Error during pruning phase: %s\n", e.what());
            return false;
        } catch (...) {
            format_and_log(0, "Unknown error during pruning phase.\n");
            return false;
        }

        format_and_log(3, "Pruning complete. Final result: Nodes=%zu, Edges=%zu\n", result_nodes->size(), result_edges->size());
    ...
    PCSTFast::NextEvent PCSTFast::get_next_event() const {
        ...
        if (edge_event_found && cluster_event_found) {
            if (next_edge_time <= next_cluster_time + kEpsilon * std::max(1.0, std::abs(next_cluster_time))) {
                format_and_log(5,"  Edge event is next (or tie).\n");
                ...
                format_and_log(5,"  Cluster event is next.\n");
            ...
            format_and_log(5,"  Only Edge event found.\n");
            event.type = NextEvent::Type::EDGE;
            event.time = next_edge_time;
            event.cluster_index = next_edge_cluster_index;
            event.edge_part_index = next_edge_part_index;
        } else if (cluster_event_found) {
            format_and_log(5,"  Only Cluster event found.\n");
            event.type = NextEvent::Type::CLUSTER_DEACTIVATION;
            event.time = next_cluster_time;
            event.cluster_index = next_cluster_deactivation_index;
        } else {
    ...
    void PCSTFast::process_event(const NextEvent& event) {
        ...
        if (event.type == NextEvent::Type::EDGE) {
            ...
            if (event.cluster_index < 0 || static_cast<size_t>(event.cluster_index) >= clusters_.size()) {
                format_and_log(2, "Warning: Edge event cluster index %d invalid. Skipping.\n", event.cluster_index);
                clusters_next_edge_event_.delete_element(event.cluster_index);
                return;
            }
            if (!clusters_[event.cluster_index].active) {
                format_and_log(4, "Edge event for inactive cluster %d. Removing from global PQ. Skipping logic.\n", event.cluster_index);
                clusters_next_edge_event_.delete_element(event.cluster_index);
                return;
            }
            ...
            format_and_log(5, "  Deleted Part=%d, Value=%.9g. Expected Part=%d, Value=%.9g.\n", deleted_part, deleted_val, event.edge_part_index, event.time);
            ...
            if (edge_parts_[event.edge_part_index].deleted) {
                ...
                format_and_log(4, "Edge part %d already deleted (flag=true), skipping event logic.\n", event.edge_part_index);
                return;
            }
            ...
            if (!clusters_deactivation_.delete_min(&deleted_val, &deleted_cluster)) {
                format_and_log(2, "Warning: Failed to delete cluster deactivation event from PQ (expected cluster %d). Maybe already merged?\n", event.cluster_index);
                if (event.cluster_index < 0 || static_cast<size_t>(event.cluster_index) >= clusters_.size() || !clusters_[event.cluster_index].active) {
                    format_and_log(4, "Skipping cluster deactivation logic as cluster %d is invalid or already inactive.\n", event.cluster_index);
                    return;
                }
                ...
                format_and_log(5, "  Deleted Cluster=%d, Value=%.9g. Expected Cluster=%d, Value=%.9g.\n", deleted_cluster, deleted_val, event.cluster_index, event.time);
                if (deleted_cluster != event.cluster_index || std::abs(deleted_val - event.time) > kEpsilon * std::max(1.0, std::abs(event.time))) {
                    format_and_log(2, "Warning: Mismatch in cluster deactivation event deletion (possibly due to prior merge). Proceeding with target cluster %d.\n", event.cluster_index);
                }
                if (event.cluster_index < 0 || static_cast<size_t>(event.cluster_index) >= clusters_.size() || !clusters_[event.cluster_index].active) {
                    format_and_log(4, "Skipping cluster deactivation logic as target cluster %d is invalid or already inactive.\n", event.cluster_index);
                    return;
                }
            }
    ...
    void PCSTFast::handle_edge_event(PayloadType edge_part_index) {
        ...
        if (!opt_info) {
            format_and_log(2, "Warning: Could not get valid edge processing info for part %d. Skipping.\n", edge_part_index);
            return;
        }
        EdgeProcessingInfo info = *opt_info;

        format_and_log(4, "Edge Event Details: Edge %d (%d <-> %d), Cost %.9g\n",
                       info.edge_idx, edges_[info.edge_idx].first, edges_[info.edge_idx].second, info.cost);
        format_and_log(4, "  Part %d: Cluster %d, Sum %.9g, FinishedMoat %.9g\n",
        ...
        if (info.current_cluster_idx == info.other_cluster_idx) {
            ...
            format_and_log(4, "Clusters %d already merged. Marking other edge part %d as deleted.\n",
        ...
        if (merge_condition) {
            ...
            format_and_log(4, "Merge condition met for edge %d.\n", info.edge_idx);
            ...
            format_and_log(4, "Growth condition for edge %d.\n", info.edge_idx);
    ...
    std::optional<PCSTFast::EdgeProcessingInfo> PCSTFast::get_edge_processing_info(PayloadType edge_part_index) {
        ...
        format_and_log(5, "  Derived OtherPart=%d, EdgeIdx=%d\n", info.other_part_idx, info.edge_idx);
        ...
        {
            format_and_log(2, "Warning: Failed to find valid representatives in get_edge_processing_info.\n");
            return std::nullopt;
        }
    ...
    void PCSTFast::merge_clusters(const EdgeProcessingInfo& info) {
        ...
        if (cluster1.active) {
            ...
            if (cluster1.moat < 0.0) { format_and_log(2,"Warning: Negative moat calculated for cluster %d (%.9g). Clamping to 0.\n", cluster1_idx, cluster1.moat); cluster1.moat = 0.0; }
        ...
        if (cluster2.active) {
            ...
            if (cluster2.moat < 0.0) { format_and_log(2,"Warning: Negative moat calculated for cluster %d (%.9g). Clamping to 0.\n", cluster2_idx, cluster2.moat); cluster2.moat = 0.0; }
    ...
    void PCSTFast::handle_cluster_deactivation_event(IndexType cluster_index) {
        ...
        if (!cluster.active) {
            format_and_log(4, "Cluster %d already inactive, skipping deactivation event logic.\n", cluster_index);
            return;
        }
        ...
        if (cluster.moat < 0.0) {
            format_and_log(2,"Warning: Negative moat calculated for cluster %d (%.9g). Clamping to 0.\n", cluster_index, cluster.moat);
            cluster.moat = 0.0;
        }
        format_and_log(3, "Cluster Deactivation: Cluster %d at time %.9g (Moat size %.9g).\n",
    ...
    void PCSTFast::update_cluster_queues_post_merge(IndexType new_cluster_index, IndexType child1_index, IndexType child2_index) {
     ...
     void PCSTFast::get_sum_on_edge_part(PayloadType edge_part_index, ValueType* total_sum,
        ...
        format_and_log(5, "  Start node derived from edge part: %d\n", start_node);
        ...
        *current_cluster_index = representative_cluster;
        ...
        if (rep_cluster.active) {
            ...
            *total_sum = path_sum + (current_time_ - rep_cluster.active_start_time);
            format_and_log(5, "  Rep is active. FinishedMoatSum=%.9g, TotalSum = %.9g + (%.9g - %.9g) = %.9g\n",
            ...
            *finished_moat_sum = *total_sum;
            format_and_log(5, "  Rep is inactive. FinishedMoatSum=TotalSum = %.9g + %.9g = %.9g\n",
    ...
    PCSTFast::IndexType PCSTFast::find_representative_and_compress(IndexType start_node, ValueType& path_sum_out) {
        ...
        while (static_cast<size_t>(current_node) < clusters_.size() && clusters_[current_node].merged_into != kInvalidIndex) {
            steps++;
            format_and_log(5, "  Step %d: CurrentNode=%d, PathSum=%.9g. Storing locally.\n", steps, current_node, path_sum_out);
            ...
            format_and_log(5, "  Step %d: NextNode=%d, StepSum=%.9g (UsedSkip=%d)\n", steps, next_node, step_sum, used_skip);
        ...
        for (const auto& [visited_node_idx, sum_at_visited] : path_compression_visited_local) {
            ...
            format_and_log(5, "    Updating skip pointer for Node %d: SkipUp=%d, SkipSum=%.9g\n",
    ...
    void PCSTFast::select_initial_active_clusters(std::vector<uint8_t>& node_good_flag) {
        ...
        if (root_ >= 0) {
            ...
            if (static_cast<size_t>(root_cluster_final_index) < clusters_.size() && clusters_[root_cluster_final_index].contains_root) {
                format_and_log(3, "  Rooted case: Starting traversal from final root cluster %d.\n", root_cluster_final_index);
                cluster_queue_local.push_back(root_cluster_final_index);
            } else {
                format_and_log(2, "Warning: Could not find final cluster containing root %d. No nodes selected initially.\n", root_);
            }
        } else {
            format_and_log(3, "  Unrooted case.\n");
            format_and_log(3, "  Unrooted case: Starting traversal from all final active clusters.\n");
        ...
        while (queue_index < cluster_queue_local.size()) {
            ...
            if (static_cast<size_t>(current_cluster_index) >= clusters_.size()) {
                format_and_log(2, "Warning: Invalid cluster index %d encountered during good node traversal.\n", current_cluster_index);
    ...
    void PCSTFast::get_statistics(Statistics* s) const {
        ...
        if(s) {
            *s = stats_;
            ...
            format_and_log(2, "Warning: Null pointer passed to get_statistics.\n");
        }
