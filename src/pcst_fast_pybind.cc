...
namespace {
    ...
    ca::PCSTFast::Logger create_python_logger() {
        return []([[maybe_unused]] int level, const std::string& message) {
            py::gil_scoped_acquire acquire;
            
            try {
                py::print(message, py::arg("flush") = true);
            } catch (const py::error_already_set& e) {
                std::cerr << "Pybind11 logging failed: " << message << std::endl;
...
std::pair<py::array_t<int64_t>, py::array_t<int64_t>> pcst_fast_pybind(
    ...
    int verbosity_level) {
        ...
        ca::PCSTFast::Logger logger = create_python_logger();
