#include "pcst_fast.h" // Assuming this header defines cluster_approx::PCSTFast and related types
...
#include <utility>
#include <vector>
#include <numeric> // For std::cmp_less/greater

// Define namespaces *before* PYBIND11_MODULE if they contain helpers used by the bound function
namespace py = pybind11;
namespace ca = cluster_approx; // Assuming this namespace is used in pcst_fast.h

namespace { // Anonymous namespace for internal helpers
    // Callback function for C++ -> Python printing. Must acquire GIL.
    void output_function(const char* output) {
        py::gil_scoped_acquire acquire; // Acquire GIL before calling Python API
        py::print(output, py::arg("flush") = true);
        // GIL is released automatically when 'acquire' goes out of scope.
    }

    // Safely casts int64_t from Python to internal int, throwing Python error if out of range.
    int safe_cast_int64_to_int(int64_t val) {
        if (std::cmp_less(val, std::numeric_limits<int>::min()) ||
            std::cmp_greater(val, std::numeric_limits<int>::max())) {
                py::gil_scoped_acquire acquire; // Acquire GIL before throwing Python exception
...
// The core function exposed to Python.
// It prepares data, runs the C++ algorithm, and converts results back to Python.
std::pair<py::array_t<int64_t>, py::array_t<int64_t>> pcst_fast_pybind(
    ...
    int verbosity_level) {
        ...
        // --- Input Validation and Parameter Adjustment ---
        ...
        // --- C++ Algorithm Execution (GIL released by call_guard here) ---
        ...
        // --- GIL re-acquired by call_guard here ---
...
// This macro defines the module entry point. All code *before* this should be
// helper functions, namespaces, includes, etc.
